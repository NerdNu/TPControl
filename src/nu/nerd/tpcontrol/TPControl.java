...
public class TPControl extends JavaPlugin implements Listener {
    ...
    // private final TPControlListener cmdlistener = new
    ...
    public boolean onCommand(CommandSender sender, Command command, String name, String[] args) {
        ...
        try {
            ...
            case "back":
                ...
                return true;
            case "tppos":
                cmdTPPos(sender, args);
                return true;
            case "tphere":
                cmdTPHere(sender, args);
                return true;
            case "tpmode":
                cmdTPMode(sender, args);
                return true;
            case "tpallow":
                cmdTPAllow(sender);
                return true;
            case "tpdeny":
                ...
                return true;
            case "tpfriend":
                cmdTPFriend(sender, args);
                return true;
            case "tpunfriend":
                cmdTPUnfriend(sender, args);
                return true;
            case "tpblock":
                cmdBlock(sender, args);
                return true;
            case "tpunblock":
                cmdUnblock(sender, args);
                return true;
            case "warps":
                cmdWarps(sender);
                return true;
            case "warp":
                ...
                return true;
            case "setwarp":
                cmdSetWarp(sender, args);
                return true;
            case "delwarp":
                cmdDelWarp(sender, args);
                return true;
            case "cancelwarp":
                cmdCancelWarp(sender);
                return true;
            case "randloc":
                cmdRandLoc(sender, args);
                return true;
            case "home":
                return cmdHome(sender, args);
            case "sethome":
                return cmdSetHome(sender, args);
            case "delhome":
                return cmdDelHome(sender, args);
            case "listhomes":
                return cmdListHomes(sender, args);
            default:
                return false;
            }
    ...
    private void cmdTP(CommandSender sender, String[] args) {
        ...
        if (canOverride(p1, p2)) {
            ...
            mode = u2.getCalculatedMode(p1); // Get the mode to operate under
                                             // (allow/ask/deny)
        }
    ...
    private void cmdRandLoc(CommandSender sender, String[] args) {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            // Calculate y offset
            double y = p.getWorld().getHighestBlockYAt((int) x, (int) z) + 0.1;
            Location dest = new Location(p.getWorld(), x, y, z);

            // If we have a region manager, ensure our location is not inside a
            // region
            if (regionManager != null) {
                BlockVector3 vector = BlockVector3.at(dest.getBlockX(), dest.getBlockY(), dest.getBlockZ());
    ...
    /**
     * Cast a command sender to a player. If not, throw a FormattedUserException
     * explaining that only players are allowed.
     * 
     * @param sender
     * @return A player
     */
